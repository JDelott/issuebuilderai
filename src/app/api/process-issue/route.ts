...
async function generateProfessionalIssue(commentText: string) {
  try {
...
export async function POST(request: Request) {
  try {
    ...
    const { commentText, repoOwner, repoName } = body;

    if (!commentText || !repoOwner || !repoName) {
    ...
    const issueContent = await generateProfessionalIssue(commentText);
